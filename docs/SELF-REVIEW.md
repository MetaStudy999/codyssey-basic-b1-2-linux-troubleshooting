...
- Final deterministic validation: `PASS`
...
| Final repository validator | PASS | Runs `31217849075` and `31217903709` completed successfully. |
...
| final repository validation | `31217849075`, `31217903709` | PASS |
...
- Fix: `monitor.sh` prefers the PID owning `AGENT_PORT` via `ss -lntp`; pattern matching is only fallback and excludes ancestor runner processes.
- Verification: actual runtime monitor follows the port-listening worker PID and records RSS/CPU/thread state.
- Final status: `RESOLVED`

### SR-002 — One combined run did not isolate Deadlock from CPU protection

- Original severity: `MAJOR` for deadlock evidence quality.
- Symptom: broad settings allowed CPU protection behavior to interfere before a clean deadlock comparison was observed.
- Fix: focused deadlock probe used allowed settings `MEMORY_LIMIT=512`, `CPU_MAX_OCCUPY=10` and varied only `MULTI_THREAD_ENABLE`.
- Result: mutual lock cycle, futex waits, PID/resource stall and true/false comparison.
- Final status: `RESOLVED`

### SR-003 — `ps %CPU` lifetime average was insufficient for short CPU bursts

- Original severity: `MAJOR` for CPU evidence precision.
- Symptom: `ps %CPU` did not track the application's short periodic load bursts reliably.
- Fix: read-only `/proc/<pid>/stat` tick-delta sampler at ~0.25 second intervals, scoped to the supplied app process group.
- Result: target listener/worker showed interval CPU rise while application CpuWorker telemetry rose above 50% and triggered protection termination.
- Final status: `RESOLVED`

### SR-004 — Mission example wording differs from supplied CPU build

- Severity: `MAJOR` if fabricated.
- Observation: supplied build produced `CPU Threshold Violated!` and exit 143, but no literal `[WATCHDOG]` or `SIGTERM` application text.
- Decision: do not fabricate missing strings. Use `Watchdog` only as the Mission protection-policy concept and quote the actual build signature.
- Final status: `RESOLVED`
...
- deterministic GitHub Actions runs, actual Runtime Evidence, final self-review and PR diff inspection are used as compensating verification;
- if an external reviewer is later available, their review should remain limited to BLOCKER/MAJOR, false PASS, requirement omission and secret exposure.

This tooling limitation is not an official Mission requirement and does not replace or weaken the actual Runtime/Evidence checks.

## Final Gate-4 Verdict

`BLOCKER=0`, `MAJOR=0` after all findings were resolved. Deterministic final validation passed. The Workcell is ready for G8 merge.
